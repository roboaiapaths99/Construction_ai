...
CAMERA_RTSP_URL = "rtsp://192.168.1.16:554/11"  # IP camera URL
...
RECOGNITION_ENABLED = os.getenv("RECOGNITION_ENABLED", "true").lower() == "true"
...
async def startup_event():
    ...
    # Start AI worker for safety violation detection
    if AI_ENABLED:
        start_ai_worker()
...
async def get_workers():
    ...
    try:
        workers = list(db["workers"].find({}, {"_id": 1, "name": 1, "role": 1, "status": 1, "location": 1}).sort("name", 1))
        
        for w in workers:
            w["_id"] = str(w.get("_id", ""))
            w["worker_id"] = w["_id"]  # Add worker_id for frontend compatibility
...
async def delete_worker(worker_id: str):
    ...
    try:
        ...
        worker = db["workers"].find_one({"_id": obj_id})
        if not worker:
            raise HTTPException(status_code=404, detail="Worker not found")
        
        # Delete worker
        db["workers"].delete_one({"_id": obj_id})
        
        # Delete face profiles (if employee_face_profiles collection exists)
        try:
            db["employee_face_profiles"].delete_many({"employee_id": obj_id})
        except:
            pass  # Collection might not exist
        
        # Delete attendance records (if attendance_logs collection exists)
        try:
            db["attendance_logs"].delete_many({"employee_id": obj_id})
        except:
            pass  # Collection might not exist
...
async def configure_camera(source: str = Form(...), custom_url: str = Form(None)):
    ...
    try:
        ...
        camera_config["camera_type"] = source
        camera_config["custom_url"] = custom_url or ""
        
        return {"success": True, "message": "Camera configured successfully"}
    except Exception as e:
...
def generate_mjpeg_stream():
    ...
    # Use camera source based on configuration
    camera_source = camera_config.get("camera_source", "webcam")
    
    if camera_source == "webcam":
        # Use webcam index 0 with MSMF backend for Windows
        cap = cv2.VideoCapture(0, cv2.CAP_MSMF)
    else:
        # Use custom URL for IP camera
        custom_url = camera_config.get("custom_url", "")
        if custom_url:
    ...
    try:
        while True:
            ...
            if not ret:
                ...
                if camera_config.get("camera_source") == "webcam":
                    cap = cv2.VideoCapture(0, cv2.CAP_MSMF)
                else:
                    custom_url = camera_config.get("custom_url", "")
                    if custom_url:
