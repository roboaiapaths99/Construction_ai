...
class AIConfig:
    ...
    # PPE Detection Model (Hansung-Cho/yolov8-ppe-detection)
    PPE_MODEL_REPO = "Hansung-Cho/yolov8-ppe-detection"
    PPE_MODEL_CACHE_DIR = "models"
    
    @staticmethod
    def get_model_path():
        """Get PPE model path, downloading if necessary"""
        try:
            model_path = hf_hub_download(
                repo_id=AIConfig.PPE_MODEL_REPO,
                filename="best.pt",
                cache_dir=AIConfig.PPE_MODEL_CACHE_DIR
            )
            return model_path
        except Exception as e:
            print(f"❌ Failed to download PPE model: {e}")
            # Fallback to environment variable or default
            return os.getenv("MODEL_PATH", "../ai/models/yolov8n.pt")
    
    # Model Settings
    MODEL_PATH = os.getenv("MODEL_PATH", None)  # Will use get_model_path() if None
    ...
    # PPE Detection Classes (from Hansung-Cho/yolov8-ppe-detection model)
    DETECTION_CLASSES = {
        0: "Hardhat",
        1: "Mask",
        2: "NO-Hardhat",
