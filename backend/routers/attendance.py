...
async def list_employees(status: Optional[str] = None):
    ...
    try:
        ...
        print(f"📋 Returning {len(employees)} employees")
...
async def delete_employee(employee_id: str):
    ...
    try:
        ...
        db.employee_face_profiles.delete_many({"employee_id": ObjectId(employee_id)})
        
        # Delete attendance records
        db.attendance_logs.delete_many({"employee_id": ObjectId(employee_id)})
...
async def get_attendance_status():
    ...
    try:
        ...
        today_count = db.attendance_logs.count_documents({"date": today})
        ...
        return {
            ...
            "attendance_records_today": today_count
        }
...
async def quick_enroll(
    ...
    """
    Quick enrollment - create employee and enroll face in one step
    """
    try:
        logger.info(f"📝 Enrollment request received: name={name}, email={email}, phone={phone}")
        db = get_db()
        if db is None:
        ...
        if quality_score < 0.4:
            raise HTTPException(status_code=400, detail=f"Face quality too low ({quality_score:.2f})")
        ...
        face_profile = {
            ...
            "quality_score": float(quality_score) if isinstance(quality_score, (np.floating, float)) else quality_score,
            "is_primary": True,
            "created_at": datetime.utcnow()
        }
        
        db.employee_face_profiles.insert_one(face_profile)
        ...
        logger.error(f"Error in quick enrollment: {e}")
        raise HTTPException(status_code=500, detail=f"Error: {str(e)}")
